...
with st.expander("Preview catalog data"):
    ...
    """if len(df) > 200:
        preview_df = df[preview_cols].sample(n=200, random_state=None)
        st.caption(f"Showing a random sample of 200 of {len(df):,} rows.")
    else:
...
if question:
    ...
    with st.chat_message("assistant"):
        ...
        st.markdown(reply)
        badge = "🟢 pandas (instant, free)" if source == "pandas" else "🔵 LLM fallback"
